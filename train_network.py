...
class NetworkTrainer:
    ...
    def train(self, args):
        ...
        num_of_steps = len(train_dataset_group) # train_dataloader batch size is 1, so this is equivalent to len(train_dataloader)
        # 学習ステップ数を計算する
        ...
        sharded_dataloader = train_dataloader
        ...
        metadata = {
            ...
            "ss_num_batches_per_epoch": num_of_steps ,
        ...
        if initial_step > 0:
            if args.skip_until_initial_step:
                ...
                if args.resume_from_epoch and epoch_from_state is not None:
                    ...
                    epoch_to_start = initial_step // math.ceil(num_of_steps/ args.gradient_accumulation_steps)
            else:
        ...
        # training loop
        dataset_was_modified_during_skip = False # Flag to track if dataset was modified during epoch skipping
        if initial_step > 0:  # only if skip_until_initial_step is specified
            ...
            for skip_epoch in range(epoch_to_start):  # skip epochs
                ...
                dataset_was_modified_during_skip = True
            ...
            if dataset_was_modified_during_skip:
                logger.info("Recreating dataloader due to dataset modification during epoch skipping for resume/initial_step.")
                # Ensure n_workers, current_epoch, current_step, and ds_for_collator are correctly scoped or redefined if necessary.
                # n_workers is defined earlier. current_epoch and current_step are Value objects.
                # ds_for_collator might need to be redefined based on the updated train_dataset_group.
                ds_for_collator = train_dataset_group if args.max_data_loader_n_workers == 0 else None
                collator = train_util.collator_class(current_epoch, current_step, ds_for_collator)
                ...
                # Important: The sharded_dataloader that will be used by the training loop
                # needs to be updated to this newly prepared dataloader.
                ...
                # sharded_dataloader = accelerator.prepare(train_dataloader) # Removed from here
            ...
            # a prepared dataloader with a non-prepared one.
            
            # Re-prepare sharded_dataloader if initial_step > 0, using the potentially updated train_dataloader
            # sharded_dataloader = accelerator.prepare(train_dataloader) # Removed from here
          

            # Moved train_dataloader creation here to create dataloader after finalizing train_dataset_group and caching as necessary.
        # sharded_dataloader = train_dataloader # This line is removed.

        # Unconditionally prepare the sharded_dataloader before the epoch loop.
        # train_dataloader will be:
        # 1. The initially prepared dataloader (if initial_step <= 0).
        # 2. The initially prepared dataloader (if initial_step > 0 and dataset_was_modified_during_skip is False).
        # 3. The new raw dataloader (if initial_step > 0 and dataset_was_modified_during_skip is True).
